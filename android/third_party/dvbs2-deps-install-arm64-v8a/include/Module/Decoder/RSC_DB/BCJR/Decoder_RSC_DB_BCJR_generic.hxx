...
Decoder_RSC_DB_BCJR_generic<B, R, MAX>::Decoder_RSC_DB_BCJR_generic(const int K,
...
Decoder_RSC_DB_BCJR_generic<B, R, MAX>::clone() const
{
...
template<typename B, typename R, tools::proto_max<R> MAX>
void
Decoder_RSC_DB_BCJR_generic<B, R, MAX>::__fwd_recursion(const R* sys, const R* par)
{
    for (auto k = 0; k < this->K / 2; k++)
    {
        for (auto s = 0; s < this->n_states; s++)
        {
            ...
            this->alpha[k + 1][s] = MAX(
              MAX(this->alpha[k][s0] + this->gamma[k][4 * s0 + 0], this->alpha[k][s1] + this->gamma[k][4 * s1 + 1]),
              MAX(this->alpha[k][s2] + this->gamma[k][4 * s2 + 2], this->alpha[k][s3] + this->gamma[k][4 * s3 + 3]));
        }
...
Decoder_RSC_DB_BCJR_generic<B, R, MAX>::__bwd_recursion(const R* sys, const R* /*par*/, R* ext)
{
    for (auto k = this->K / 2 - 1; k >= 0; k--)
    {
        ...
        {
            post0 = MAX(post0,
                        this->alpha[k][s] + this->gamma[k][4 * s + 0] + this->beta[k + 1][this->trellis[0][4 * s + 0]]);
            post1 = MAX(post1,
                        this->alpha[k][s] + this->gamma[k][4 * s + 1] + this->beta[k + 1][this->trellis[0][4 * s + 1]]);
            post2 = MAX(post2,
                        this->alpha[k][s] + this->gamma[k][4 * s + 2] + this->beta[k + 1][this->trellis[0][4 * s + 2]]);
            post3 = MAX(post3,
                        this->alpha[k][s] + this->gamma[k][4 * s + 3] + this->beta[k + 1][this->trellis[0][4 * s + 3]]);
        }
