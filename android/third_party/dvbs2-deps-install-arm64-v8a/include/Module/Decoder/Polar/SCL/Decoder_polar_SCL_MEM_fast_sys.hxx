...
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::Decoder_polar_SCL_MEM_fast_sys(const int& K,
                                                                                const int& N,
                                                                                const int& L,
...
template<typename B, typename R, class API_polar>
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::Decoder_polar_SCL_MEM_fast_sys(
  const int& K,
  const int& N,
  const int& L,
  const std::vector<bool>& frozen_bits,
...
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::clone() const
{
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::set_frozen_bits(const std::vector<bool>& fb)
{
...
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::get_frozen_bits() const
{
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::_decode(const R* Y_N)
{
...
template<typename B, typename R, class API_polar>
int
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R, class API_polar>
int
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    if (!API_polar::isAligned(Y_N))
        throw spu::tools::runtime_error(__FILE__, __LINE__, __func__, "'Y_N' is misaligned memory.");
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::recursive_decode(const R* Y_N,
                                                                  const int off_l,
                                                                  const int off_s,
                                                                  const int rev_depth,
                                                                  int& node_id)
        ...
        switch (node_type)
        {
            case tools::polar_node_t::STANDARD:
                ...
                for (auto i = 0; i < n_active_paths; i++)
                {
                    ...
                    if (array_left != paths[i])
                    {
                        ...
                        API_polar::xo(sleft[array_left].data() + off_s,
                                      s[paths[i]].data() + off_s + n_elm_2,
                                      s[paths[i]].data() + off_s,
                                      n_elm_2);
                    }
                }
                for (auto i = 0; i < n_active_paths; i++)
                {
                    const auto array_left = path_2_array_s[paths[i]][rev_depth - 1];
                    if (array_left == paths[i])
                        API_polar::xo(s[array_left].data() + off_s,
                                      s[paths[i]].data() + off_s + n_elm_2,
                                      s[paths[i]].data() + off_s,
                                      n_elm_2);
                }
            ...
            case tools::polar_node_t::RATE_0_LEFT:
                for (auto i = 0; i < n_active_paths; i++)
                {
                    ...
                    if (is_left) path_2_array_s[paths[i]][rev_depth] = paths[i];
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::_store(B* V_K) const
{
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::_store_cw(B* V_N) const
{
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::update_paths_r0(const int r_d,
                                                                 const int off_l,
                                                                 const int off_s,
                                                                 const int n_elmts)
{
...
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::update_paths_r0(const int off_l, const int off_s)
{
    if (n_active_paths > 1)
        for (auto i = 0; i < n_active_paths; i++)
        {
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::update_paths_r1(const int r_d,
                                                                 const int off_l,
                                                                 const int off_s,
                                                                 const int n_elmts)
{
...
template<int REV_D, int N_ELMTS>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::update_paths_r1(const int off_l, const int off_s)
{
    if (REV_D == 0)
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::flip_bits_r1(const int old_path,
                                                              const int new_path,
                                                              const int dup,
                                                              const int off_s,
                                                              const int /*n_elmts*/)
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::update_paths_rep(const int r_d,
                                                                  const int off_l,
                                                                  const int off_s,
                                                                  const int n_elmts)
{
...
template<int REV_D, int N_ELMTS>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::update_paths_rep(const int off_l, const int off_s)
{
    constexpr B b = spu::tools::bit_init<B>();
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::update_paths_spc(const int r_d,
                                                                  const int off_l,
                                                                  const int off_s,
                                                                  const int n_elmts)
{
...
template<int REV_D, int N_ELMTS>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::update_paths_spc(const int off_l, const int off_s)
{
    // the number of candidates to generate per list
    const auto n_cands = L <= 2 ? 4 : 8;
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::flip_bits_spc(const int old_path,
                                                               const int new_path,
                                                               const int dup,
                                                               const int off_s,
                                                               const int /*n_elmts*/)
                                                                ...
                                                                const int r_d)
{
    ...
    std::copy(s[old_path].begin() + off_s, s[old_path].begin() + off_s + n_elmts, s[new_path].begin() + off_s);
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::delete_path(int path_id, const int r_d)
{
...
template<typename B, typename R, class API_polar>
int
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::up_ref_array_idx(const int path, const int r_d)
{
    ...
    if (n_array_ref_l[old_array][r_d] > 1)
    {
        ...
        auto new_array = 0;
        while (n_array_ref_l[new_array][r_d])
            new_array++;
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::copy_left(const int r_d, const int off_s)
{
    const auto n_elmts = 1 << r_d;
    ...
    for (auto i = 0; i < n_active_paths; i++)
    {
        ...
        if ((child_left != paths[i]) && (n_array_ref_s[paths[i]][r_d - 1] > 0))
            std::copy(s[paths[i]].begin() + off_s, s[paths[i]].begin() + off_s + n_elm_2, s2[paths[i]].begin() + off_s);
    }
}
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_MEM_fast_sys<B, R, API_polar>::erase_bad_paths(const int r_d)
{
    // erase bad paths
    auto k = 0;
