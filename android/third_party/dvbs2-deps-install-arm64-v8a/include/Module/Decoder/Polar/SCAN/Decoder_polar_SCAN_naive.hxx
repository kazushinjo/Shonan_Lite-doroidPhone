...
template<typename B,
         ...
         tools::proto_i<R> I,
         tools::proto_s<R> S>
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::Decoder_polar_SCAN_naive(const int& K,
                                                                        const int& N,
                                                                        const int& max_iter,
...
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::clone() const
{
...
template<typename B,
         ...
         tools::proto_s<R> S>
void
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::_reset(const size_t /*frame_id*/)
{
...
template<typename B,
         ...
         tools::proto_s<R> S>
void
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::_load(const R* Y_N)
{
...
template<typename B,
         ...
         tools::proto_s<R> S>
void
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::_decode()
{
...
template<typename B,
         ...
         tools::proto_s<R> S>
int
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::_decode_siho(const R* Y_N, B* V_K, const size_t /*frame_id*/)
{
...
template<typename B,
         ...
         tools::proto_s<R> S>
int
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t /*frame_id*/)
{
    //	auto t_load = std::chrono::steady_clock::now(); // -----------------------------------------------------------
    // LOAD
    this->_load(Y_N);
...
template<typename B,
         ...
         tools::proto_s<R> S>
int
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::_decode_siso(const R* Y_N1, R* Y_N2, const size_t /*frame_id*/)
{
    // ----------------------------------------------------------------------------------------------------------- LOAD
    this->_load(Y_N1);
...
template<typename B,
         ...
         tools::proto_s<R> S>
void
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::_store(B* V_KN, bool coded) const
{
    if (!coded)
    {
        auto k = 0;
        ...
        for (auto i = 0; i < this->N; i++)
            V_KN[i] = H(this->feedback_graph[this->layers_count - 1][i] + this->soft_graph[this->layers_count - 1][i]);
}
...
template<typename B,
         ...
         tools::proto_s<R> S>
void
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::set_soft_val_and_propagate(const int l, const int j, const R v)
{
...
template<typename B,
         ...
         tools::proto_s<R> S>
void
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::fb_compute_soft_output(const int& i)
{
    ...
    for (auto l = l_start; l >= 0; l--) // for each layer
    {
        ...
        for (auto j = i; j < i + j_stop; j++)
        {
            ...
            if ((i & (1 << l)) == 0) // function F
                soft_graph[l][j] =
                  F(soft_graph[l + 1][f_up_ix], V(soft_graph[l + 1][f_down_ix], feedback_graph[l][f_down_ix]));
            else // function G
                soft_graph[l][j] =
                  V(soft_graph[l + 1][g_down_ix], F(feedback_graph[l][g_up_ix], soft_graph[l + 1][g_up_ix]));
        }
    }
}
...
template<typename B,
         ...
         tools::proto_s<R> S>
void
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::set_n_frames(const size_t n_frames)
{
    if (n_frames != 1)
    {
        std::stringstream message;
    ...
    if (old_n_frames != n_frames) Decoder_SISO<B, R>::set_n_frames(n_frames);
}
...
template<typename B,
         ...
         tools::proto_s<R> S>
void
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::set_frozen_bits(const std::vector<bool>& fb)
{
...
Decoder_polar_SCAN_naive<B, R, F, V, H, I, S>::get_frozen_bits() const
{
