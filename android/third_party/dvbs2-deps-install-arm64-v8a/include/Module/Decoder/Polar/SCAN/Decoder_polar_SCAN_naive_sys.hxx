...
template<typename B,
         ...
         tools::proto_i<R> I,
         tools::proto_s<R> S>
Decoder_polar_SCAN_naive_sys<B, R, F, V, H, I, S>::Decoder_polar_SCAN_naive_sys(const int& K,
                                                                                const int& N,
                                                                                const int& max_iter,
...
Decoder_polar_SCAN_naive_sys<B, R, F, V, H, I, S>::clone() const
{
...
template<typename B,
         ...
         tools::proto_s<R> S>
int
Decoder_polar_SCAN_naive_sys<B, R, F, V, H, I, S>::_decode_siso(const R* sys,
                                                                const R* par,
                                                                R* ext,
...
template<typename B,
         ...
         tools::proto_s<R> S>
void
Decoder_polar_SCAN_naive_sys<B, R, F, V, H, I, S>::_store(B* V_KN, bool coded) const
{
    if (!coded)
    {
        auto k = 0;
        for (auto i = 0; i < this->N; i++)
            if (!this->frozen_bits[i]) // if i is not a frozen bit
                V_KN[k++] =
                  H(this->feedback_graph[this->layers_count - 1][i] + this->soft_graph[this->layers_count - 1][i]);
    }
    else
        for (auto i = 0; i < this->N; i++)
