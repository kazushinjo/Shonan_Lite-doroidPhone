...
Decoder_RSC_BCJR_inter_intra<B, R>::Decoder_RSC_BCJR_inter_intra(const int& K,
...
Decoder_RSC_BCJR_inter_intra<B, R>::clone() const
{
...
template<typename B, typename R>
void
Decoder_RSC_BCJR_inter_intra<B, R>::_load(const R* Y_N)
{
...
template<typename B, typename R>
int
Decoder_RSC_BCJR_inter_intra<B, R>::_decode_siso_alt(const R* sys, const R* par, R* ext, const size_t /*frame_id*/)
{
...
template<typename B, typename R>
void
Decoder_RSC_BCJR_inter_intra<B, R>::_store(B* V_K) const
{
...
{
    static mipp::Reg<R> apply(mipp::Reg<R> r) { return mipp::div2(r); }
};
...
{
    static mipp::Reg<R> compute(const mipp::Reg<R>& r_post) { return r_post; }
};
...
{
    static mipp::Reg<signed char> compute(const mipp::Reg<signed char>& r_post) { return r_post.sat(-63, 63); }
};
...
{
    static mipp::Reg<short> apply(const mipp::Reg<short>& r_metrics,
...
{
    static mipp::Reg<signed char> apply(const mipp::Reg<signed char>& r_metrics,
                                        const mipp::Reg<signed char>& r_cmask_norm,
                                        const int& i = 0)
    {
        (void)i;
