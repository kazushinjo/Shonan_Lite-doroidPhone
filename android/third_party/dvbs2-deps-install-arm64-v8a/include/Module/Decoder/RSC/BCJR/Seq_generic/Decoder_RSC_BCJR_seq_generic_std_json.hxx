...
Decoder_RSC_BCJR_seq_generic_std_json<B, R, RD, MAX1, MAX2>::Decoder_RSC_BCJR_seq_generic_std_json(
  const int& K,
...
template<typename B, typename R, typename RD, tools::proto_max<R> MAX1, tools::proto_max<RD> MAX2>
void
Decoder_RSC_BCJR_seq_generic_std_json<B, R, RD, MAX1, MAX2>::compute_ext(const R* sys, R* ext)
{
    ...
    for (auto i = 0; i < this->K + this->n_ff; i++)
    {
        ...
        for (auto j = 0; j < this->n_states; j++)
            if (this->trellis[1][j] == +1)
            {
                ...
                max0_json = std::max(max0_json,
                                     (RD)(this->alpha[j][i] + this->beta[this->trellis[6][j]][i + 1] +
            ...
            else
            {
                ...
                max1_json = std::max(max1_json,
                                     (RD)(this->alpha[j][i] + this->beta[this->trellis[8][j]][i + 1] -
        ...
        if (i < this->K) ext[i] = RSC_BCJR_seq_generic_post<R, RD>::compute(max0 - max1) - sys[i];

        transition_a = (max0_json > max1_json) ? transition_a0 : transition_a1;
        transition_b = (max0_json > max1_json) ? transition_b0 : transition_b1;

        stream << "\t\t\t\t\t\t\t"
               << "[" << std::setw(2) << +transition_a << ", " << +transition_b << "]";
        if (i != (this->K + this->n_ff - 1)) stream << "," << std::endl;
    }
}
