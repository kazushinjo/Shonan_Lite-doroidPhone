...
Decoder_RSC_BCJR_seq_generic<B, R>::Decoder_RSC_BCJR_seq_generic(const int& K,
  ...
  , gamma(2)
{
    ...
    for (unsigned i = 0; i < alpha.size(); i++)
        alpha[i].resize(K + this->n_ff + 1);
    for (unsigned i = 0; i < beta.size(); i++)
        beta[i].resize(K + this->n_ff + 1);
    for (unsigned i = 0; i < gamma.size(); i++)
        gamma[i].resize(K + this->n_ff + 0);
...
Decoder_RSC_BCJR_seq_generic<B, R>::clone() const
{
...
{
    static R apply(R m) { return spu::tools::div2<R>(m); }
};
...
{
    static R compute(const RD& post) { return post; }
};
...
{
    static signed char compute(const RD& post) { return (signed char)spu::tools::saturate<RD>(post, -63, 63); }
};
