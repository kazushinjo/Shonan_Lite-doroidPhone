...
Decoder_RSC_BCJR_seq_scan<B, R, RD>::Decoder_RSC_BCJR_seq_scan(const int& K,
...
Decoder_RSC_BCJR_seq_scan<B, R, RD>::clone() const
{
...
template<typename B, typename R, typename RD>
void
Decoder_RSC_BCJR_seq_scan<B, R, RD>::compute_gamma(const R* sys, const R* par)
{
...
{
    static void
    apply(short& ab0, short& ab1, short& ab2, short& ab3, short& ab4, short& ab5, short& ab6, short& ab7, const int i)
    {
        // normalization
        if (i % 8 == 0)
        {
...
{
    static void apply(signed char& ab0,
                      signed char& ab1,
                      signed char& ab2,
                      signed char& ab3,
                      signed char& ab4,
...
template<typename B, typename R, typename RD>
void
Decoder_RSC_BCJR_seq_scan<B, R, RD>::compute_alpha()
{
    ...
    for (auto w = 1; w < alpha_vec_loop_range; w += window)
    {
        ...
        {
            const R alpha_origin[8] = { this->alpha[0][w + i - 1], this->alpha[1][w + i - 1], this->alpha[2][w + i - 1],
                                        this->alpha[3][w + i - 1], this->alpha[4][w + i - 1], this->alpha[5][w + i - 1],
            ...
            for (auto j = 0; j < block; j++)
            {
                ...
                // normalization
                RSC_BCJR_seq_scan_normalize2<R>::apply(this->alpha[0][w + i + j],
                                                       this->alpha[1][w + i + j],
                                                       this->alpha[2][w + i + j],
                                                       this->alpha[3][w + i + j],
                                                       this->alpha[4][w + i + j],
    ...
    for (auto i = alpha_vec_loop_range; i < this->K + 3; i++) // naive traversal (sequential)
    {
        ...
        // normalization
        RSC_BCJR_seq_scan_normalize2<R>::apply(this->alpha[0][i],
                                               this->alpha[1][i],
                                               this->alpha[2][i],
                                               this->alpha[3][i],
                                               this->alpha[4][i],
...
template<typename B, typename R, typename RD>
void
Decoder_RSC_BCJR_seq_scan<B, R, RD>::compute_beta()
{
    constexpr int window = WINDOW;
    constexpr int block = BLOCK; // block have to be a multiple of window
    ...
    for (auto w = this->K + 2; w > beta_vec_loop_range; w -= window)
    {
        ...
        for (auto i = 0; i < window; i += block) // SCAN implementation part2 (sequential)
        {
            const R beta_origin[8] = { this->beta[0][w - i + 1], this->beta[1][w - i + 1], this->beta[2][w - i + 1],
                                       this->beta[3][w - i + 1], this->beta[4][w - i + 1], this->beta[5][w - i + 1],
            ...
            for (auto j = 0; j < block; j++)
            {
                ...
                // normalization
                RSC_BCJR_seq_scan_normalize2<R>::apply(this->beta[0][w - (i + j)],
                                                       this->beta[1][w - (i + j)],
                                                       this->beta[2][w - (i + j)],
                                                       this->beta[3][w - (i + j)],
                                                       this->beta[4][w - (i + j)],
    ...
    for (auto i = beta_vec_loop_range; i > 0; i--) // naive traversal (sequential)
    {
        ...
        // normalization
        RSC_BCJR_seq_scan_normalize2<R>::apply(this->beta[0][i],
                                               this->beta[1][i],
                                               this->beta[2][i],
                                               this->beta[3][i],
                                               this->beta[4][i],
...
template<typename B, typename R, typename RD>
void
Decoder_RSC_BCJR_seq_scan<B, R, RD>::compute_ext(const R* sys, R* ext)
{
