...
Decoder_RSC_BCJR_seq_generic_std<B, R, RD, MAX1, MAX2>::Decoder_RSC_BCJR_seq_generic_std(
  const int& K,
  const std::vector<std::vector<int>>& trellis,
  const bool buffered_encoding)
  : Decoder_RSC_BCJR_seq_generic<B, R>(K, trellis, buffered_encoding)
  , beta_prev(trellis[0].size())
  , beta_cur(trellis[0].size())
{
...
Decoder_RSC_BCJR_seq_generic_std<B, R, RD, MAX1, MAX2>::clone() const
{
...
template<typename B, typename R, typename RD, tools::proto_max<R> MAX1, tools::proto_max<RD> MAX2>
void
Decoder_RSC_BCJR_seq_generic_std<B, R, RD, MAX1, MAX2>::compute_gamma(const R* sys, const R* par)
{
...
template<typename B, typename R, typename RD, tools::proto_max<R> MAX1, tools::proto_max<RD> MAX2>
void
Decoder_RSC_BCJR_seq_generic_std<B, R, RD, MAX1, MAX2>::compute_alpha()
{
    // compute alpha values [trellis forward traversal ->]
    for (auto i = 1; i < this->K + this->n_ff; i++)
    {
        for (auto j = 0; j < this->n_states; j++)
            this->alpha[j][i] = MAX1(this->alpha[this->trellis[0][j]][i - 1] +
                                       this->trellis[1][j] * this->gamma[this->trellis[2][j]][(i - 1)],
                                     this->alpha[this->trellis[3][j]][i - 1] +
                                       this->trellis[4][j] * this->gamma[this->trellis[5][j]][(i - 1)]);
...
template<typename B, typename R, typename RD, tools::proto_max<R> MAX1, tools::proto_max<RD> MAX2>
void
Decoder_RSC_BCJR_seq_generic_std<B, R, RD, MAX1, MAX2>::compute_beta()
{
    // compute beta values [trellis backward traversal <-]
    for (auto i = this->K + this->n_ff - 1; i >= 1; i--)
    {
        for (auto j = 0; j < this->n_states; j++)
            this->beta[j][i] = MAX1(this->beta[this->trellis[6][j]][i + 1] + this->gamma[this->trellis[7][j]][i],
...
template<typename B, typename R, typename RD, tools::proto_max<R> MAX1, tools::proto_max<RD> MAX2>
void
Decoder_RSC_BCJR_seq_generic_std<B, R, RD, MAX1, MAX2>::compute_ext(const R* sys, R* ext)
{
    ...
    for (auto i = 0; i < this->K; i++)
    {
        ...
        for (auto j = 0; j < this->n_states; j++)
            if (this->trellis[1][j] == +1)
                max0 = MAX2(max0,
                            (RD)this->alpha[j][i] + (RD)this->beta[this->trellis[6][j]][i + 1] +
                              (RD)this->gamma[this->trellis[7][j]][i]);
            else
                max1 = MAX2(max1,
                            (RD)this->alpha[j][i] + (RD)this->beta[this->trellis[8][j]][i + 1] -
...
template<typename B, typename R, typename RD, tools::proto_max<R> MAX1, tools::proto_max<RD> MAX2>
void
Decoder_RSC_BCJR_seq_generic_std<B, R, RD, MAX1, MAX2>::compute_beta_ext(const R* sys, R* ext)
{
    ...
    {
        for (auto j = 0; j < this->n_states; j++)
            beta_cur[j] = MAX1(beta_prev[this->trellis[6][j]] + this->gamma[this->trellis[7][j]][i],
    ...
    {
        RD max0 = (RD)this->alpha[0][i] + (RD)beta_prev[this->trellis[6][0]] + (RD)this->gamma[this->trellis[7][0]][i];

        RD max1 = (RD)this->alpha[0][i] + (RD)beta_prev[this->trellis[8][0]] - (RD)this->gamma[this->trellis[9][0]][i];

        for (auto j = 1; j < this->n_states; j++)
            if (this->trellis[1][j] == 1)
                max0 = MAX2(max0,
                            (RD)this->alpha[j][i] + (RD)beta_prev[this->trellis[6][j]] +
                              (RD)this->gamma[this->trellis[7][j]][i]);
            else
                max1 = MAX2(max1,
                            (RD)this->alpha[j][i] + (RD)beta_prev[this->trellis[8][j]] -
                              (RD)this->gamma[this->trellis[9][j]][i]);
...
template<typename B, typename R, typename RD, tools::proto_max<R> MAX1, tools::proto_max<RD> MAX2>
void
Decoder_RSC_BCJR_seq_generic_std<B, R, RD, MAX1, MAX2>::compute_ext_sys(const R* sys, R* ext_sys)
{
    // compute extrinsic values
    for (auto i = 0; i < this->K + this->n_ff; i++)
    {
...
template<typename B, typename R, typename RD, tools::proto_max<R> MAX1, tools::proto_max<RD> MAX2>
void
Decoder_RSC_BCJR_seq_generic_std<B, R, RD, MAX1, MAX2>::compute_ext_par(const R* par, R* ext_par)
{
    // compute extrinsic values
    for (auto i = 0; i < this->K + this->n_ff; i++)
    {
...
template<typename B, typename R, typename RD, tools::proto_max<R> MAX1, tools::proto_max<RD> MAX2>
int
Decoder_RSC_BCJR_seq_generic_std<B, R, RD, MAX1, MAX2>::_decode_siso_alt(const R* sys,
                                                                         const R* par,
                                                                         R* ext,
...
template<typename B, typename R, typename RD, tools::proto_max<R> MAX1, tools::proto_max<RD> MAX2>
int
Decoder_RSC_BCJR_seq_generic_std<B, R, RD, MAX1, MAX2>::_decode_siso(const R* Y_N1, R* Y_N2, const size_t /*frame_id*/)
{
