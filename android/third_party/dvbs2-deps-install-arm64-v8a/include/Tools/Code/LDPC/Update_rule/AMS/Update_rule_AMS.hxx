...
Update_rule_AMS<R, MIN>::Update_rule_AMS()
  : name("AMS")
  , sign(0)
  , min(std::numeric_limits<R>::max())
  , delta_min(min)
  , delta(min)
  , n_ite(0)
  , ite(0)
...
template<typename R, proto_min<R> MIN>
std::string
Update_rule_AMS<R, MIN>::get_name() const
{
    return this->name;
}
...
template<typename R, proto_min<R> MIN>
void
Update_rule_AMS<R, MIN>::begin_decoding(const int n_ite)
{
    this->n_ite = n_ite;
}
...
template<typename R, proto_min<R> MIN>
void
Update_rule_AMS<R, MIN>::begin_ite(const int ite)
{
    this->ite = ite;
}
...
template<typename R, proto_min<R> MIN>
void
Update_rule_AMS<R, MIN>::end_chk_node_in()
{
...
template<typename R, proto_min<R> MIN>
void
Update_rule_AMS<R, MIN>::end_chk_node_out()
{
}

template<typename R, proto_min<R> MIN>
void
Update_rule_AMS<R, MIN>::end_ite()
{
}

template<typename R, proto_min<R> MIN>
void
Update_rule_AMS<R, MIN>::end_decoding()
{
}
}
}
