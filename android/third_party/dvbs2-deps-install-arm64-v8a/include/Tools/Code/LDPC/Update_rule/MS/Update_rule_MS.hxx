...
{
template<typename R>
Update_rule_MS<R>::Update_rule_MS()
  : name("MS")
  , sign(0)
  , min1(std::numeric_limits<R>::max())
  , min2(min1)
  , cst1(0)
  , cst2(0)
  , n_ite(0)
...
template<typename R>
inline std::string
Update_rule_MS<R>::get_name() const
{
    return this->name;
}
...
template<typename R>
inline void
Update_rule_MS<R>::begin_decoding(const int n_ite)
{
    this->n_ite = n_ite;
}
...
template<typename R>
inline void
Update_rule_MS<R>::begin_ite(const int ite)
{
    this->ite = ite;
}
...
template<typename R>
inline void
Update_rule_MS<R>::end_chk_node_in()
{
...
template<typename R>
inline void
Update_rule_MS<R>::end_chk_node_out()
{
}

template<typename R>
inline void
Update_rule_MS<R>::end_ite()
{
}

template<typename R>
inline void
Update_rule_MS<R>::end_decoding()
{
}
}
}
