...
namespace tools
{
Pattern_polar_i ::Pattern_polar_i(const int& N,
  ...
  , max_level(max_level)
{
    ...
    if (node == nullptr) throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, "'node' can't be null.");
    ...
    for (auto i = 0; i < node->get_depth(); i++)
        *const_cast<int*>(p_size) >>= 1;

    const int* p_si_2 = &si_2;
    *const_cast<int*>(p_si_2) = size >> 1;

    const int* p_off_l = &off_l;
    for (auto i = N; i > (N >> node->get_depth()); i >>= 1)
        *const_cast<int*>(p_off_l) += i;

    const int* p_off_s = &off_s;
    *const_cast<int*>(p_off_s) = node->get_lane_id() * size;
}

Pattern_polar_i ::Pattern_polar_i(const int min_level, const int max_level)
  : N(0)
  , m(0)
  , size(0)
  , si_2(0)
...
void
Pattern_polar_i ::set_id(const unsigned id)
{
    this->id = id;
}

unsigned
Pattern_polar_i ::get_id()
{
    return this->id;
}

void
Pattern_polar_i ::set_key(const std::string& key)
{
    this->key = key;
}

std::string
Pattern_polar_i ::get_key()
{
    return this->key;
}

int
Pattern_polar_i ::get_size()
{
    return size;
}

int
Pattern_polar_i ::get_si_2()
{
    return si_2;
}

int
Pattern_polar_i ::get_off_l()
{
    return off_l;
}

int
Pattern_polar_i ::get_off_s()
{
    return off_s;
}

int
Pattern_polar_i ::get_min_lvl()
{
    return min_level;
}

int
Pattern_polar_i ::get_max_lvl()
{
    return max_level;
}
...
int
Pattern_polar_i ::match(const int& reverse_graph_depth, const Binary_node<Pattern_polar_i>* node_curr)
{
    if (reverse_graph_depth <= 0)
    {
        std::stringstream message;
        message << "'reverse_graph_depth' has to be greater than 0 ('reverse_graph_depth' = " << reverse_graph_depth
                << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
