...
void
f_seq<R, F, 0>::apply(const R* __restrict l_a, const R* __restrict l_b, R* __restrict l_c, const int n_elmts)
{
...
void
g_seq<B, R, G, N_ELMTS>::apply(const R* __restrict l_a,
...
void
g_seq<B, R, G, 0>::apply(const R* __restrict l_a,
                         const R* __restrict l_b,
                         const B* __restrict s_a,
                         R* __restrict l_c,
                         const int n_elmts)
...
void
g0_seq<R, G0, 0>::apply(const R* __restrict l_a, const R* __restrict l_b, R* __restrict l_c, const int n_elmts)
{
    for (auto i = 0; i < n_elmts; i++)
        l_c[i] = G0(l_a[i], l_b[i]);
}
...
template<typename B, typename R, proto_g<B, R> G, int N_ELMTS>
void
gr_seq<B, R, G, N_ELMTS>::apply(const R* __restrict l_a,
                                const R* __restrict l_b,
                                const B* __restrict s_a,
                                R* __restrict l_c,
                                const int /*n_elmts*/)
...
template<typename B, typename R, proto_g<B, R> G>
void
gr_seq<B, R, G, 0>::apply(const R* __restrict l_a,
                          const R* __restrict l_b,
                          const B* __restrict s_a,
                          R* __restrict l_c,
                          const int n_elmts)
...
void
h_seq<B, R, H, 0>::apply(const R* __restrict l_a, B* __restrict s_a, const int n_elmts)
{
    for (auto i = 0; i < n_elmts; i++)
        s_a[i] = H(l_a[i]);
}
...
template<typename B>
void
h0_seq<B, 0>::apply(B* __restrict s_a, const int n_elmts)
{
    std::fill(s_a, s_a + n_elmts, 0);
}
...
template<typename B, typename R, proto_h<B, R> H>
void
rep_seq<B, R, H, 0>::apply(const R* __restrict l_a, B* __restrict s_a, const int n_elmts)
{
    R sum_l = (R)0;
    for (auto i = 0; i < n_elmts; i++)
        sum_l += l_a[i];
...
void
rep_seq<B, signed char, H, 0>::apply(const signed char* __restrict l_a, B* __restrict s_a, const int n_elmts)
{
    int sum_l = 0; // accumulate in a integer instead of a char
    for (auto i = 0; i < n_elmts; i++)
        sum_l += (int)l_a[i];
...
spc_seq<B, R, H, N_ELMTS>::apply(const R* __restrict l_a, B* __restrict s_a, const int /*_elmts*/)
{
    ...
    if (prod_sign < 0) s_a[cur_min_pos] = (s_a[cur_min_pos] == 0) ? spu::tools::bit_init<B>() : 0; // correction
...
template<typename B, typename R, proto_h<B, R> H>
bool
spc_seq<B, R, H, 0>::apply(const R* __restrict l_a, B* __restrict s_a, const int n_elmts)
{
    auto cur_min_abs = std::numeric_limits<R>::max();
    auto cur_min_pos = -1;
    auto prod_sign = 1;
...
void
xo_seq<B, X, 0>::apply(const B* __restrict s_a, const B* __restrict s_b, B* __restrict s_c, const int n_elmts)
{
    for (auto i = 0; i < n_elmts; i += 1)
        s_c[i] = X(s_a[i], s_b[i]);
}
