...
void
g_intra_unaligned<B, R, GI>::apply(const R* __restrict l_a,
                                   const R* __restrict l_b,
...
template<typename B, typename R, proto_g_i<B, R> GI>
void
gr_intra_unaligned<B, R, GI>::apply(const R* __restrict l_a,
                                    const R* __restrict l_b,
                                    const B* __restrict s_a,
                                    R* __restrict l_c,
                                    const int /*n_elmts*/)
...
template<typename B, typename R, proto_h_i<B, R> HI>
void
rep_intra<B, R, HI, 0>::apply(const R* __restrict l_a, B* __restrict s_a, const int n_elmts)
{
    constexpr auto stride = mipp::nElmtsPerRegister<R>();
...
spc_intra<B, R, HI, N_ELMTS>::apply(const R* __restrict l_a, B* __restrict s_a, const int /*n_elmts*/)
{
    ...
    if (s_prod_sign < 0) // make the correction
    {
        ...
        while (std::abs(l_a[i]) != s_cur_min_abs)
            i++;
...
template<typename B, typename R, proto_h_i<B, R> HI>
bool
spc_intra<B, R, HI, 0>::apply(const R* __restrict l_a, B* __restrict s_a, const int n_elmts)
{
    constexpr auto stride = mipp::nElmtsPerRegister<R>();
