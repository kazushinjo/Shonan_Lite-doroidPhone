...
// ★Android移植向けの変更点(dvbs2_tx_lib.cppと同じ理由)。
...
// ★Rx開始直後の急激なメモリ増加の原因箇所を特定するためのデバッグ専用マクロ。
#define AV_MEMLOG(label) \
	std::cerr << "[MEMDBG] " << (label) << " mem=" << shonan_debug_memory_mb() << "MB" << std::endl

using namespace aff3ct;
using namespace aff3ct::module;

// ★datv_dvbs2_rx_runはブロッキング実行(waiting/learning/transmissionの各フェーズを
// 内部ループで回り続ける)で、Swift側からは入出力FIFOを閉じることしかできない。
// FIFOを閉じてもこのループ自体は止まらずCPU/メモリを消費し続けるため
// (実機でstop()後もメモリが際限なく増加する事象を確認)、外部から停止を要求できる
// フラグを追加し、各フェーズのstop conditionラムダで参照する。
static std::atomic<bool> g_dvbs2_rx_stop_requested{false};

extern "C" void datv_dvbs2_rx_request_stop()
{
	g_dvbs2_rx_stop_requested.store(true);
}

// ★sink(TS出力)はBCH/LDPCの復号成否に関わらず毎フレーム無条件で書き込む(BER/FER計測用の
// シミュレーションハーネス構造のため)。よって「TSバイトが出力FIFOに届いているか」は
// ロック状態の指標にならない(信号なし/ノイズのみでも一定レートで出力され続ける、実機で
// 確認済み)。frame synchronizerの相関値がしきい値を超えているか(sync_frame->get_packet_flag()、
// aff3ct内部でFLGとして扱われている値そのもの)を直接ロック指標として公開する。
static std::atomic<bool> g_dvbs2_rx_locked{false};

extern "C" bool datv_dvbs2_rx_is_locked()
{
	return g_dvbs2_rx_locked.load();
}

// #define MULTI_THREADED // comment this line to disable multi-threaded RX
// ★実機で"Run OTAC scheduler"の組合せ最適化計算が完了せずアプリがハングする事象を確認。
// (プロファイリングフェーズ自体はUSER_BIN Radioのブロッキング読み込みを伴う実データ処理で
// 完了するが、続くschedule()は統計値のみを使う純粋な数値計算にもかかわらず、完了しなかった)。
// まずは安定動作を優先し、スケジューラを使わないシングルスレッド実行(Sequence直接実行)に
// 切り替える。スループットはTx同様マルチスレッド化するより低くなるが、安定性を優先する。
// #define ENABLE_PROBES // comment this line to disable the probes in the chain

// global parameters
constexpr bool enable_logs = true;
#ifdef MULTI_THREADED
// iOSのサンドボックスでは低レベルなCPUアフィニティ制御(スレッドピニング)は許可されないため
// 無効化する(dvbs2_tx_lib.cpp/DVBS2_iOS_Port_PoC.md参照)。
constexpr bool thread_pinning = false;
...
// 元は int main(int argc, char** argv)。iOSアプリからライブラリ関数として呼び出せるように
// extern "C" 関数へ変更した(dvbs2_tx_lib.cppと同じ方針)。
extern "C" int datv_dvbs2_rx_run(int argc, char** argv)
{
// dvbs2_tx_lib.cppと同じ理由(未捕捉のC++例外によるiOS実機SIGABRT回避)で関数全体を
// 例外境界で囲む。
try
{
	g_dvbs2_rx_stop_requested.store(false);
	g_dvbs2_rx_locked.store(false);
#ifdef MULTI_THREADED
	if (thread_pinning)
	{
	...
	AV_MEMLOG("after tools(cstl/itl_core/poly_gen)");
	...
	AV_MEMLOG("after source/sink/radio");
	...
	AV_MEMLOG("after itl_rx/modem/freq_shift/sync_frame/feedbr/sync_fine_lr");
	...
	AV_MEMLOG("after all modules constructed");
...
#endif
	// ★シングルスレッド実行時のtransmission phaseログ(731/745行目付近)がwaiting phaseの
	// 外からもこの変数を参照するため、関数レベルへ引き上げた(no_wl_phases=trueで
	// waiting phaseをスキップした場合も未初期化にならないようここで初期化する)。
	unsigned m = 0;
	if (!params.no_wl_phases) {
		...
		{
			m += params.n_frames;
			if (m % 10 == 0 || m <= 5) AV_MEMLOG(std::string("waiting phase m=") + std::to_string(m));
			...
			if (radio_usrp != nullptr && m % radio_flush_period == 0 && !sync_frame->get_packet_flag())
				radio_usrp->flush();
#endif
			// ★フレーム同期(get_packet_flag)が成立しないまま無期限にループし続けると、
			// 実機でごく短時間(数秒)のうちにメモリが数GB規模まで際限なく増加する事象を
			// 確認した(原因未特定、waiting phaseのループ内で毎回何らかのバッファが解放
			// されずに蓄積している可能性が高い)。同期未成立でも安全に終了できるよう、
			// 一定時間で強制タイムアウトする(DVBS2_iOS_Port_PoC.md参照)。
			std::chrono::duration<double> waiting_elapsed = std::chrono::system_clock::now() - start_waiting;
			bool waiting_timeout = waiting_elapsed.count() > 8.0;
			return sync_frame->get_packet_flag() || g_dvbs2_rx_stop_requested.load() || waiting_timeout;
		});
		...
		sequence_waiting_and_learning_1_2.exec([&](const std::vector<const int*>& statuses)
		{
			...
			return (m >= limit) || g_dvbs2_rx_stop_requested.load();
		});
		...
		(const std::vector<const int*>& statuses)
	{
		...
		terminal_stats.temp_report(stats_file);
#endif
		return g_dvbs2_rx_stop_requested.load();
	});
#else
	// start the transmission sequence
	sequence_transmission.exec(
#ifdef ENABLE_PROBES
		[&noise_est, &estimator, &sync_frame, &t_start, &params, &terminal_stats, &stats_file, &m]
#else
		[&noise_est, &estimator, &sync_frame, &t_start, &params, &m]
#endif
		(const std::vector<const int*>& statuses)
		{
			if (params.rx_time_limit)
			...
			m += params.n_frames;
			g_dvbs2_rx_locked.store(sync_frame->get_packet_flag());
			if (statuses.back() != nullptr)
			{
#ifdef ENABLE_PROBES
				terminal_stats.temp_report(stats_file);
...
{
	std::cerr << "[datv_dvbs2_rx_run] exception: " << e.what() << std::endl;
	__android_log_print(ANDROID_LOG_ERROR, "Dvbs2Native", "[datv_dvbs2_rx_run] exception: %s", e.what());
	return EXIT_FAILURE;
}
catch (...)
{
	std::cerr << "[datv_dvbs2_rx_run] unknown exception" << std::endl;
	__android_log_print(ANDROID_LOG_ERROR, "Dvbs2Native", "[datv_dvbs2_rx_run] unknown exception");
	return EXIT_FAILURE;
}
}
