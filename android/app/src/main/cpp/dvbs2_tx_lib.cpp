...
// ★Android移植向けの変更点(iOS版はstd::cerrがXcodeコンソールに出るが、Androidには
// それに相当する端末が無く例外メッセージが消えてしまうため、catch節でlogcatにも出す)。
...
// ★dvbs2_rx_lib.cppと同じ理由(Swift側からFIFOを閉じるだけでは内部のブロッキング実行
// ループが止まらずCPU/メモリを消費し続ける)で、外部から停止要求できるフラグを追加する。
static std::atomic<bool> g_dvbs2_tx_stop_requested{false};

extern "C" void datv_dvbs2_tx_request_stop()
{
	g_dvbs2_tx_stop_requested.store(true);
}

// ★スレッド数を8→4に削減してもTx/Rx同時実行時にRx側のwaiting phaseが8秒→22秒に
// 遅延する(CPU競合が残る)ことを確認した。Rx側は既にシングルスレッド版で動いている
// ため、Txも揃えてシングルスレッド化しCPU競合そのものを排除する
// (DVBS2_iOS_Port_PoC.md 12.11続報)。
...
// 無効化する(DVBS2_iOS_Port_PoC.md参照)。パイプライン自体の並列処理(マルチスレッド)は
// 引き続き有効。
const bool thread_pinnig = false;
...
// extern "C" 関数へ変更した(名前修飾を避けCブリッジ層から呼べるようにするため)。
// argc/argvはコマンドライン相当のパラメータをそのまま渡す想定(--mod-cod, --src-path等)。
extern "C" int datv_dvbs2_tx_run(int argc, char** argv)
{
// aff3ct/streampu側が投げるC++例外(コンステレーションファイル未検出等)をキャッチせずに
// 素通しすると、iOS実機ではterminate()経由でSIGABRTしアプリごと即死する。
// Swift側にエラーとして伝播できるよう、関数全体を例外境界で囲む。
try
{
	g_dvbs2_tx_stop_requested.store(false);
#ifdef MULTI_THREADED
	if (thread_pinnig)
	{
	...
	// ★Rx側はシングルスレッド版(dvbs2_rx_lib.cppでMULTI_THREADED無効)で動いているが、
	// Tx側が8スレッド(1+6+1)を専有すると、Tx/Rx同時実行時にRx側のCPU時間が確保できず
	// Rx処理(BER統計出力)が全く進まなくなる事象を確認した(DVBS2_iOS_Port_PoC.md 12.11続報)。
	// Rxにコアを残すため削減する。
	const std::vector<size_t> n_threads_per_stages = { 1, 2, 1 };
	...
	std::function<bool(const std::vector<const int*>&)> stop_condition =
		[&n_frames, &t_start, &params] (const std::vector<const int*>& statuses) {
			...
			return g_dvbs2_tx_stop_requested.load();
		};
	...
	for (auto &m : sequence_transmission.get_modules<spu::tools::Interface_waiting>())
		m->cancel_waiting();
#endif /* MULTI_THREADED */
	std::clog << std::endl;
...
{
	std::cerr << "[datv_dvbs2_tx_run] exception: " << e.what() << std::endl;
	__android_log_print(ANDROID_LOG_ERROR, "Dvbs2Native", "[datv_dvbs2_tx_run] exception: %s", e.what());
	return EXIT_FAILURE;
}
catch (...)
{
	std::cerr << "[datv_dvbs2_tx_run] unknown exception" << std::endl;
	__android_log_print(ANDROID_LOG_ERROR, "Dvbs2Native", "[datv_dvbs2_tx_run] unknown exception");
	return EXIT_FAILURE;
}
}
